...
//Class to manage general stuff going on 
public class GameManager : MonoBehaviour{
	...
	private Vector3 fishDirection;//Average position and direction of fish fish cam can follow
	...
	void Start (){
        ...
        fishList = new List<GameObject>();//Initialise fish list + Spawn leader
        ...
        leader.GetComponent<Fish>().maxSpeed--;//So followers can keep up

        followPoint = leader.transform.position - leader.transform.forward * followDistance;//Set follow point behind leader
        fishList.Add(leader);//First fish in list will always be leader
	...
	void Update (){
        ...
        //If leader isn't null update follow point
		if (leader){
        ...
        //Set this object's position and direction to fish centroid and average direction for fish cam 
	...
	void CalcFishDirection(){
		...
		if (fishList.Count > 0){
            ...
            //Divide by number of fish to get average and then normalise
	...
	void CheckForArrival(){
        ...
        //If the distance from leader to current target point is less than paths arrival radius set next point to be followed
